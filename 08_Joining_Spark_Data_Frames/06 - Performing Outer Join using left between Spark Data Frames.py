...
# MAGIC - If the users does not have any course enrolments, we need to get all user details. Course details will be substitiued with null values
# MAGIC   - Need to perform left or right outer join **users_df** and **course_enrolments_df**
...
# MAGIC     - *course_id* and *course_enrolmane_id* from *course_enrolments_df*
# MAGIC - For this example using these data frames, using just *outer* also give same results. But it is not correect to use *outer*
...
# MAGIC %md
# MAGIC - Get all the users who have not enrolled for any courses. Recommended to use primary key in the child table when comparing with null values
...
# MAGIC %md
# MAGIC - Get number of courses enrolled by each user
# MAGIC - If there are no enrollments, then count should return 0
